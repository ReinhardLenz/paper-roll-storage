📦 Paper Roll Storage & Input-Output Combination in Robotic Warehousing
This repository includes two related Python projects that simulate:

Paper Roll Storage Optimization: Visualizing input/output dynamics and batch diameter combinations.

Input-Output Combination Cycles: Analyzing how combined movements in a robotic conveyor warehouse can improve efficiency, especially when input is continuous and output is time-limited.

📘 Project 1: Paper Roll Storage
This script transforms production statistics into a diameter–width diagram, optimizing how batches of paper rolls with identical diameters are grouped and stored.


🤖 Project 2: Input-Output Combination Cycles in a Robotic Conveyor Warehouse
In a factory warehouse where:

Input = 24/7 automated production

Output = custom deliveries during limited daytime hours

…robot conveyors can sometimes combine input and output into a single movement. This simulation quantifies how much of that combined movement is realistically possible depending on the number of output hours per day.
...
This situation is modeled probabilistically:

Input probability = 1/3

Output probability = 2/3
(similar to rolling a die with 2/3 blue sides for output and 1/3 red for input)

![0-dices](https://github.com/ReinhardLenz/input_output_combine/assets/71219487/481da8f6-5277-4aa8-8be2-2a8948e0d924)


📊 Storage Fill Rate Visualization
...
The top graph shows storage content (zigzag pattern), while the bottom graph shows pcs/hour over time:

Blue = constant input

Red = output concentrated during day
To maintain balance, Area_input = Area_output over 24h.
...
📐 Probability Tree Visualized in Scale

![2-puu2](https://github.com/ReinhardLenz/input_output_combine/assets/71219487/8d3d1b3c-21dc-4260-a66c-0fcb931864a5)

Each event sequence is represented with its probability on the x-axis and number of output/input/combined steps on the y-axis:
...
📏 Area = Probability
Each area (width × height) corresponds to the total event probability:
...
Output cycle: 1.00 / 1.66 ≈ 60%

Input cycle: 0.44 / 1.66 ≈ 26.5%

Combined: 0.22 / 1.66 ≈ 13.5%
...
Combined probability curve continues to rise

Graph becomes too wide for tree diagram, but numeric results still valid
...
⏰ Varying Output Time During Day

![9-time-var](https://github.com/ReinhardLenz/input_output_combine/assets/71219487/3d9d74b3-f5a8-4791-9143-a5804ae4d529)

This theoretical curve shows how the probabilities change as daytime output hours vary:

Fewer output hours → fewer combination opportunities

More output hours → more balance, but combined cycles never reach 100% due to stochastic limitations

🧠 Insights
Even with 24h input and 12h output, only ~25% of cycles can be combined

Extending output time improves efficiency, but randomness prevents full optimization

Tree diagrams and stair-step curves help visualize these probabilities quantitatively
